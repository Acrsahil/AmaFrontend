...
export function useKitchenWebSocket(
    ...
    kitchentypeId?: string | number | null
) {
    ...
    const connect = useCallback(() => {
        ...
        // Delay opening the socket until both branchId and kitchentypeId are available
        if (!branchId || !kitchentypeId) {
            console.log("[WS] Delaying kitchen socket connection: branchId or kitchentypeId is missing", {
                branchId,
                kitchentypeId,
            });
            return;
        ...
        let wsUrl = `${WS_BASE_URL}/ws/kitchen/${branchId}/${kitchentypeId}/`;
        if (token) wsUrl += `?token=${token}`;

        console.log("[WS] Creating new kitchen socket with URL:", wsUrl);
